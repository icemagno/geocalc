...
public class AircraftClass {
	...
	public AircraftObject update( AttributeHandleValueMap theAttributes, ObjectInstanceHandle theObject ) throws Exception {
		for ( AircraftObject aircraft : instances ) {
			if( aircraft.isMe( theObject ) ) {
				for( AttributeHandle attributeHandle : theAttributes.keySet() )	{
					if( attributeHandle.equals( positionHandle) ) {
						aircraft.setPosition( encoder.decodePosition( theAttributes.get(attributeHandle) ) );
					}
				}
				return aircraft;
			}
	...
	public void updateAttributeValuesObject( AircraftObject aircraft ) throws Exception {
		...
		if ( rtiamb.isAttributeOwnedByFederate(aircraft.getHandle(), positionHandle) ) {
	...
	public void subscribe() throws RTIexception {
		AttributeHandle positionHandle = rtiamb.getAttributeHandle( aircraftHandle, "Position" );
